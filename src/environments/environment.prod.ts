export const environment = {
  production: true,
  login: false,
  appID: {
    clientId: '03e82b0d-a368-42a6-9d81-9a0391e8592d',
    discoveryEndpoint: 'https://us-south.appid.cloud.ibm.com/oauth/v4/ff2d5da9-4499-4ef0-9fc6-7e280c05ea9c/.well-known/openid-configuration'
  },
  websocket: 'https://banking-0-alpha.apps.cluster-9ddp2.9ddp2.sandbox2472.opentlc.com',
  backend: 'https://banking-0-alpha.apps.cluster-9ddp2.9ddp2.sandbox2472.opentlc.com'
};


